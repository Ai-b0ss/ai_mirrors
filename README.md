...
Optional secrets depend on enabled integrations, for example `OPENAI_API_KEY`,
`HF_TOKEN`, `HF_DATASET`, `NOTION_WS_JSON`, proxy settings, and search provider
keys.
...
- [TL;DR](#-tldr--суть-за-минуту)
- [Идентификация проекта](#-идентификация-проекта)
- [Инфраструктура и топология](#-инфраструктура-и-топология)
- [Архитектура: 6 слоёв](#-архитектура-6-слоёв)
- [Слой 1 — транспорт и прокси](#-слой-1--транспорт-telegram-и-прокси)
- [Слой 2 — шлюз моделей](#-слой-2--шлюз-моделей)
- [Слой 3 — модели, каналы и авто-роутер](#-слой-3--модели-каналы-и-авто-роутер-20)
- [Слой 4 — надёжность](#-слой-4--надёжность)
- [Слой 5 — поиск и DeepResearch](#-слой-5--поиск-20-и-deepresearch-research)
- [Слой 6 — состояние и персистентность](#-слой-6--состояние-и-персистентность)
- [Команды и фичи](#-команды-и-фичи)
- [/brain — оркестратор «Мегамозг»](#-brain--оркестратор-мегамозг)
- [Интеграция с Notion](#-интеграция-с-notion-задача--fable-5)
- [Пул-ротатор воркспейсов](#-пул-ротатор-воркспейсов)
- [Безопасность и переменные окружения](#-безопасность-секреты-и-env)
- [Зависимости и QA](#-зависимости-тестирование-и-qa)
- [Развитие](#-видение-и-план-развития)
- [Перенос / деплой](#-чеклист-переноса--деплоя)
- [Глоссарий](#-глоссарий)
...
1. **Транспорт.** Telegram ↔ Cloudflare Worker ↔ long-polling. Прокси-пулы для исходящих запросов к моделям/поиску.
2. **Шлюз моделей.** Единая точка `api.byesu.com` (OpenAI-совместимый): `/v1` — OpenAI + Claude, `/v1beta` — нативный Gemini.
3. **Авто-роутер.** `classify_task` → `_capability_score` → выбор канала/модели; `ModelHealth` (circuit breaker); цепочки фоллбэка.
4. **Надёжность.** SSE-стриминг, watchdog (`HARD_CAP_SECS=300`), пул генерации (12 воркеров), ретраи, локи состояния.
5. **Веб-поиск / DeepResearch.** 7+ провайдеров → RRF-слияние → SimHash/Jaccard-дедуп → MMR → судья источников → ответ с цитатами.
6. **Состояние.** Локальный JSON + бэкап в HF Dataset.
...
1. **Вертикальный роутинг + RRF** (`k=60`), дедуп по `canonicalize_url`. Источники: Tavily, Brave, SearXNG, OpenAlex, Hacker News, GitHub (за ENV-ключами); DDG — фолбэк, Wikipedia — последний.
2. **Каскад извлечения текста.** HTML: `trafilatura` → `readability-lxml` → `BeautifulSoup` → regex. PDF: `PyMuPDF` → `pypdf` → `pdfplumber`. JS/paywall: `r.jina.ai`. YouTube-транскрипты.
3. **Семантический дедуп + MMR.** SimHash-64 (Hamming ≤ 3) или Jaccard ≥ 0.85; MMR (λ = 0.7 fast / 0.6 deep).
4. **Нейро-реранк кросс-энкодером** (`cross-encoder/mmarco-mMiniLMv2-L12-H384-v1`). По умолчанию **выключен** (`RERANK_ENABLED=0`) — требует `sentence-transformers`/`torch`; без них мягко деградирует.
5. **Семантический префильтр e5-small** (`intfloat/multilingual-e5-small`). По умолчанию **выключен** (`EMBED_ENABLED=0`) — те же тяжёлые зависимости.
6. **Умный LLM-судья.** При нейро-скорах пропускает LLM-вызов; для свежих запросов остаётся.
7. **Пофактовые цитаты + grounding.** Каждое утверждение несёт `[N]`; verify-pass проверяет опору.
8. **Агентная декомпозиция.** Bounded-DAG: 2–3 под-вопроса (`AGENTIC_MAX_SUBQ=3`).
...
<details>
<summary><b>Полный список ENV (для переноса)</b></summary>

**Транспорт/ядро:** `TG_API_WORKER`, `BOT_TOKEN`, `KEY_CLAUDE`, `KEY_CLAUDE_KIRO`, `KEY_GPT_PRO`, `KEY_GPT_PLUS`, `KEY_GEMINI`, `GEMINI_VIA_OPENAI`, `GEMINI_DISABLED`.

**Прокси:** `PROXY_1024_PASS`, `PROXY_1024_REGIONS/SCHEME/TTL`, `PROXY_FALLBACK`, `PROXY_FALLBACK_JSON`, `PROXY_AUTO`, `PROXY_DIRECT_FALLBACK`, `PROXY_DIRECT_AFTER`, `PROXY_PROBE_URL`, `PROXY_REGION`.

**Поиск:** `SEARCH_DEBUG`, `RRF_K`, `SIMHASH_MAX_DIST`, `JACCARD_THR`, `MMR_LAMBDA_FAST/DEEP`, `MMR_TOP_N_FAST/DEEP`, `JUDGE_KEEP_FAST/DEEP`, `AGENTIC_MAX_SUBQ`, `RESEARCH_ROUNDS`, `VERIFY_CTX_CHARS`, `AGENTIC_RESEARCH`, `JUDGE_TRUST_RANK`, `VERIFY_GROUNDED`, `RERANK_ENABLED`, `RERANK_FAST/DEEP`, `RERANK_MODEL`, `RERANK_MAX_PAIRS`, `RERANK_BLEND`, `EMBED_ENABLED`, `EMBED_MODEL`, `EMBED_TOP_N`, `EMBED_BATCH`, `EMBED_BACKEND`, `BRAVE_API_KEY`, `SEARXNG_URL/TOKEN`, `OPENALEX_MAILTO/API_KEY`, `GITHUB_TOKEN`.

**Дедлайны/пул:** `DEADLINE_CHEAP/MID/TOP/FLASH`, `CHANNEL_DEAD_TTL`, `GEN_POOL`.

**/brain:** `BRAIN_BUDGET_USD`, `HTTP_ATTEMPT_TIMEOUT`.

**Notion:** `NOTION_TOKEN`, `NOTION_DB_ID` или `NOTION_WS_JSON` / `NOTION_WS_n_*`, `ROTATION_STRATEGY`, `WS_COST_*`, `WS_SOFT_FACTOR`, `WS_CREDIT_LIMIT`, `WS_POOL_STATE_FILE`.
...
<sub>Документ — живая сводка. При изменениях кода/архитектуры обновляй соответствующий слой и реестр фиксов.</sub>
